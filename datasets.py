...
class Cityscapes(Dataset):
    ...
    def __init__(self, root, split='train', mode='fine', resize=None, crop=None):
        self.root = root
    ...
    def __getitem__(self, index):
        ...
        if self.crop != None:
            ...
            target = TF.center_crop(target, self.crop)

        img_tensor = transforms.ToTensor()(img)
        target = transforms.ToTensor()(target) * 255
        target = target.int()

        target_tensor = torch.zeros((19, target.size()[1], target.size()[2]), dtype=torch.float32)

        for class_ in self.classes:
            if not class_.ignore_in_eval:
                target_tensor[class_.train_id][target[0] == class_.id] = 1.
